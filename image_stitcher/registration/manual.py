"""Manual tile registration module with GUI alignment tools.

This module provides interactive GUI-based tools for manual tile alignment and
fixed shift registration. Separate from automated registration workflows.

Key components:
- AlignmentTile: Data class for tile alignment
- DraggableTileItem: Interactive Qt graphics item for tile manipulation
- FixedRegistrationDialog: Main GUI dialog for manual alignment
- Helper functions for channel detection, pattern analysis, and shift calculation

Usage:
    from image_stitcher.registration.manual import launch_fixed_registration
    success = launch_fixed_registration(input_directory)
"""
    ...
    try:
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            for tp_name, coords_file in timepoint_coords:
                ...
                # For now, save coordinates unchanged (manual alignment already applied via dialog)
                print(f"\nWARNING: Fixed shift correction disabled (technical debt)")
                print(f"  Stage motor drift pattern must be characterized first")
